...
REM ; check ว่า run As admin หรือไม่ 
cd %systemroot%\system32
call :IsAdmin

REM ; ปิด browser ที่ run อยู่ก่อน
taskkill.exe /F /IM iexplore.exe /T
taskkill.exe /F /IM msedge.exe /T


REM ; ตั้งค่าใน Advance
...
REM ; เอาติ๊กถูกออกจาก compatibiliy list
...
REM ;-----------------------------------------------------------------------------
REM ; ทำหน้าโฮมเป็น rdsrv.rd.go.th (ie11 windows7)
...
REM ;-----------------------------------------------------------------------------
REM ; เปลี่ยน Search engine เป็น google (ie11)
...
rem ; ตั้ง powercfg ให้ high performance (windows10 11)
...
rem ; ให้สิทธิเขียนไฟล์ใน temp (แก้ 5201 error)
